...
public class AdminDashboardView {
    ...
    private void createView() {
        try {
            ...
            try {
                ...
                e.printStackTrace();
                dashboardTab.setContent(createErrorContent(e.getMessage()));
            }
            dashboardTab.setClosable(false);
            
            // Utilisateurs
            Tab utilisateursTab = new Tab("Utilisateurs");
            try {
                utilisateursTab.setContent(createUtilisateursContent());
            } catch (Exception e) {
                System.err.println("ERREUR utilisateurs: " + e.getMessage());
                e.printStackTrace();
                utilisateursTab.setContent(createErrorContent(e.getMessage()));
            }
            utilisateursTab.setClosable(false);
            
            // Agents Terrain
            Tab agentsTerrainTab = new Tab("Agents Terrain");
            try {
                agentsTerrainTab.setContent(createAgentsTerrainContent());
            } catch (Exception e) {
                System.err.println("ERREUR agents terrain: " + e.getMessage());
                e.printStackTrace();
                agentsTerrainTab.setContent(createErrorContent(e.getMessage()));
            }
            agentsTerrainTab.setClosable(false);
            
            // Agents Villageois
            Tab agentsVillageoisTab = new Tab("Agents Villageois");
            try {
                agentsVillageoisTab.setContent(createAgentsVillageoisContent());
            } catch (Exception e) {
                System.err.println("ERREUR agents villageois: " + e.getMessage());
                e.printStackTrace();
                agentsVillageoisTab.setContent(createErrorContent(e.getMessage()));
            }
            agentsVillageoisTab.setClosable(false);
            
            // AVEC
            Tab avecTab = new Tab("AVEC");
            try {
                avecTab.setContent(createAvecContent());
            } catch (Exception e) {
                System.err.println("ERREUR AVEC: " + e.getMessage());
                e.printStackTrace();
                avecTab.setContent(createErrorContent(e.getMessage()));
            }
            ...
            try {
                ...
                System.err.println("ERREUR membres: " + e.getMessage());
                e.printStackTrace();
                membresTab.setContent(createErrorContent(e.getMessage()));
            }
            ...
            try {
                ...
                e.printStackTrace();
                statsTab.setContent(createErrorContent(e.getMessage()));
            }
            statsTab.setClosable(false);
            
            tabPane.getTabs().addAll(dashboardTab, utilisateursTab, agentsTerrainTab, 
                                      agentsVillageoisTab, avecTab, membresTab, statsTab);
            
            root.setCenter(tabPane);
            
        } catch (Exception e) {
            System.err.println("ERREUR CRITIQUE dans AdminDashboardView: " + e.getMessage());
            e.printStackTrace();
            ...
            Label errorLabel = new Label("Erreur lors du chargement du dashboard:");
    ...
    private VBox createSidebar() {
        ...
        menuBox.getChildren().addAll(
            ...
            createMenuButton(ICONE_UTILISATEURS, "Utilisateurs", () -> tabPane.getSelectionModel().select(1)),
            createMenuButton(ICONE_AGENTS_TERRAIN, "Agents Terrain", () -> tabPane.getSelectionModel().select(2)),
            createMenuButton(ICONE_AGENTS_VILLAGEOIS, "Agents Villageois", () -> tabPane.getSelectionModel().select(3)),
            createMenuButton(ICONE_AVEC, "AVEC", () -> tabPane.getSelectionModel().select(4)),
            createMenuButton(ICONE_MEMBRES, "Membres", () -> tabPane.getSelectionModel().select(5)),
            createMenuButton(ICONE_STATISTIQUES, "Statistiques", () -> tabPane.getSelectionModel().select(6))
        );
    ...
    private VBox createDashboardContent() {
        ...
        try {
            nbAvec = avecService.getStatistiques().getTotalAvecs();
        ...
        actionsBox.setStyle("-fx-background-color: " + Styles.BLANC + ";" +
    ...
    private VBox createUtilisateursContent() {
        ...
        supprimerBtn.setStyle("-fx-background-color: " + Styles.ROUGE_ERREUR + "; -fx-text-fill: white; -fx-cursor: hand;");
    ...
    private VBox createAgentsTerrainContent() {
        ...
        content.getChildren().addAll(toolbar, agentTerrainTable);
        return content;
    }
    ...
    private VBox createAgentsVillageoisContent() {
        ...
        agentVillageoisTable.getColumns().addAll(colId, colNom, colPrenom, colEmail, colAgentTerrain);
        
        chargerAgentsVillageois();
        
        content.getChildren().addAll(toolbar, agentVillageoisTable);
        return content;
    }
    ...
    private VBox createMembresContent() {
        ...
        searchField.setPromptText("Rechercher un membre par nom...");
        ...
        ajouterBtn.setStyle(Styles.BOUTON_SECONDAIRE);
        ajouterBtn.setOnAction(e -> showAjoutMembre());
        
        toolbar.getChildren().addAll(searchField, searchButton, ajouterBtn);
        ...
        TableColumn<Membre, String> colAvec = new TableColumn<>("AVEC");
        colAvec.setCellValueFactory(cellData -> {
            try {
                Membre m = cellData.getValue();
                if (m.getAvecId() != null) {
                    Avec avec = avecService.getAvecById(m.getAvecId());
                    return new javafx.beans.property.SimpleStringProperty(avec != null ? avec.getNom() : "");
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
        ...
        colRole.setCellValueFactory(cellData -> 
            new javafx.beans.property.SimpleStringProperty(cellData.getValue().getRoleComite().getDescription()));
        
        TableColumn<Membre, String> colStatut = new TableColumn<>("Statut");
        colStatut.setCellValueFactory(cellData -> 
            new javafx.beans.property.SimpleStringProperty(cellData.getValue().getStatut().getLibelle()));
        
        membreTable.getColumns().addAll(colId, colNom, colPrenom, colCarte, colAvec, colRole, colStatut);
    ...
    private VBox createStatsContent() {
        ...
        statsGrid.setStyle("-fx-background-color: " + Styles.BLANC + ";" +
                          "-fx-background-radius: 10;");
        ...
        try {
            ...
            int nbAgentsTerrain = agentTerrainService.getNombreAgentTerrain();
            int nbAgentsVillageois = agentVillageoisService.getNombreAgentVillageois();
            int nbAvec = avecService.getStatistiques().getTotalAvecs();
            ...
            // Statistiques par AVEC
            List<Avec> avecs = avecService.getAllAvecs();
            int totalMembresParAvec = 0;
            for (Avec avec : avecs) {
                totalMembresParAvec += membreService.getMembresByAvecId(avec.getId()).size();
            }
            ...
            System.err.println("Erreur chargement stats: " + e.getMessage());
            statsGrid.add(createStatItem("⚠️", "Erreur", e.getMessage()), 0, 0);
        }
    ...
    private VBox createStatItem(String icon, String label, String value) {
        ...
        item.setStyle("-fx-background-color: " + Styles.GRIS_CLAIR + ";" +
                     "-fx-background-radius: 10;");
        ...
        Label valueLabel = new Label(value);
        valueLabel.setStyle("-fx-font-size: 18px; -fx-font-weight: bold; -fx-text-fill: " + Styles.VERT_PRINCIPAL + ";");
    ...
    private void rechercherMembres(String recherche) {
        ...
        try {
            // Recherche dans toutes les AVEC
            List<Avec> avecs = avecService.getAllAvecs();
    ...
    private void showAjoutMembre() {
        ...
        content.getChildren().addAll(
            ...
            new Label("AVEC:"), avecCombo,
            new Label("Profession:"), professionField,
            new Label("Village:"), villageField,
    ...
    private void showAjoutAgentVillageois() {
        ...
        content.getChildren().addAll(
            ...
            new Label("Mot de passe:"), passwordField,
            new Label("Téléphone:"), telephoneField,
            new Label("Agent terrain:"), agentTerrainCombo
        );
    ...
    private void showAjoutAvec() {
        ...
        formFields.getChildren().addAll(
            ...
            tauxLabel, tauxField,
            caisseSolidariteCheck, cotisationBox,
            dureeLabel, dureeSpinner
        );
